jQuery(document).ready(function($) {
    'use strict';
    var this_obj = pool_services_lite_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.pool-services-lite-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message pool-services-lite-plugin-install');
                button.addClass('button-primary pool-services-lite-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.pool-services-lite-recommended-plugins .pool-services-lite-activation-note').css('display','block');
    ...
    $(document).on('click', '.pool-services-lite-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .pool-services-lite-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .pool-services-lite-recommended-plugins').css('display','none');
    });
